 ...
 * CSV 读取上下文.
 * <p>
 * 提供链式调用接口用于配置 CSV 读取参数.
 *
 * @param <T> 数据类型
 * @author Jax Jiang
 */
@Accessors(chain = true, fluent = true)
public class CsvReadContext<T> extends AbstractReadContext<T> {
	 ...
	 * @param clazz 数据类型
	 */
	public CsvReadContext(ExcelReadService service, InputStream inputStream, Class<T> clazz) {
	...
	public void performRead(ExcelReaderBuilder builder) {
		if (this.readConfig != null && this.readConfig.getHeadRowNumber() != null) {
			builder.headRowNumber(this.readConfig.getHeadRowNumber());
		}
		builder.csv().doRead();
	}

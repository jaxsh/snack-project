...
> 审计日期：2026-05-24 | 最后更新：2026-05-30（GAP-6 修复）
...
```
  ...
  └─────────────┘                  │                                              │
                                   ...
                                   ┌────────────────▼─────────────────────────────┐
                                   │  Database                                    │
                                   │  oauth2_user / oauth2_authorization ← SAS   │
...
```
/api/oauth2/user/profile → authenticated()         首匹配；pre_auth_reset 用户可访问
/api/oauth2/user/**      → hasAuthority("SCOPE_upms")  仅服务账号可访问
...
前端携带 Session Cookie → Order 2 链（OAuth2 Client）处理认证和 AT 管理 → 业务请求本身**不带 Bearer 头**，Controller 直接处理。Order 3 链（Resource Server）主要生效于内部服务调用：UPMS 使用 `snack-upms-service` client_credentials AT 携带 Bearer 头访问 oauth-server 管理端点（`/api/oauth2/user/**`）。

---

## 2. 请求流转

### 2.1 用户登录（浏览器 → OAuth2 Client → SAS）

```
浏览器                  OAuth2 Client (Order 2)              SAS (Order 1)
  │                            │                                │
  │ 访问受保护页面              │                                │
  │───────────────────────────>│                                │
  │                            │ 无 Session，重定向             │
...
```
浏览器              OAuth2 Client (Order 2)       Resource Server (Order 3)    Controller
  │                        │                              │                        │
  │ GET /api/upms/users    │                              │                        │
  │ (Session Cookie)       │                              │                        │
  │───────────────────────>│                              │                        │
...
OAuth2 Client (SessionStateCheckFilter)              SAS (OAuth2TokenCustomizerImpl)
  ...
  │ → session.invalidate()                                   │
  │ → HTTP 401 JSON {"code":"2003","data":{"loginUrl":"..."}}│
```
...
```
SysUserServiceImpl              OAuth2UserClient                oauth-server /api/oauth2/user/**
  │                                    │                                    │
  │ create(dto)                        │                                    │
  │───────────────────────────────────>│                                    │
...
```
/api/oauth2/user/profile → authenticated()
    首匹配；pre_auth_reset 用户也可访问（用于加载改密状态）

/api/oauth2/user/**（其余） → hasAuthority("SCOPE_upms")
...
管理员 POST /api/upms/users
    ...
    │   ③ OAuth2UserClient → POST /api/oauth2/user（snack-upms-service AT）
    │       ④ 创建 oauth2_user：
...
oauth2_user 与 sys_user 双写，OAuth2UserClient 调用失败时通过 `transactionTemplate` 回滚 UPMS 侧。
...
| 触发条件 | 字段 |
|---------|------|
| 初始密码 | `oauth2_user.initialPassword = 1` |
...
① SAS 颁发受限 AT：
   ...
   OAuth2TokenCustomizerImpl → scope={pre_auth_reset}, authorities=[], TTL=5min

② 授权码回调 → BFF 建立 Session（持有受限 AT）

③ GET /api/oauth2/user/profile
   /profile 走第一条规则 authenticated() → 放行
   响应：{initialPassword:1} 或 {expired:1}

④ app.tsx onPageChange 路由守卫：
   initialPassword===1 或 expired===1 → history.replace('/account/change-password')
   改密页副标题："请修改密码以继续使用"

⑤ 用户提交新密码 → PUT /api/oauth2/user/profile
   PasswordRestrictionAuthorizationManager：/profile 走第一条规则 → 放行
   OAuth2UserServiceImpl.update():
       initialPassword=0, lastPasswordResetTime=now()

⑥ 前端：
   await changePassword(values.password)          // 改密
   const { redirectUrl } = await logout()          // 撤销 Token + 销毁 Session
   await message.success('密码修改成功，请重新登录') // 等提示展示完（3秒）
   window.location.href = redirectUrl              // 重走 OAuth2 授权码流程
```
...
AT 到期，下次请求触发刷新：
    ...
    ④ SessionStateCheckFilter 捕获 OAuth2AuthorizationException
         ...
         HTTP 401：{"code":"2003","data":{"loginUrl":"..."}}
    ⑤ 前端显示"登录状态已过期，请重新登录"，1.5秒后跳转
...
第 5 次失败 → lockUser()：
    ...
    DB: oauth2_user.locked=1, lockCount, lockUntil 更新
...
管理员通过以下接口管控账号，UPMS 服务层按需同步至 oauth2_user：

| 操作 | 接口 | oauth2_user 副作用 |
|------|------|-------------------|
...
| 强制下线 | `DELETE /api/upms/users/{id}/tokens` | 按 `principal_name` 删除 `oauth2_authorization`（RT 立即失效） |
...
POST /logout
    ...
    │    oauth2_authorization 标记无效；OAuth2AuthorizedClientRepository 移除记录
    │
    ...
    前端: window.location.href = redirectUrl → 重新走授权码流程
```

**补充**：Token 撤销后 AT 在 Resource Server 侧最长仍有 5 分钟有效期（纯 JWT 验证，不查 SAS 是否已撤销）。`redirectUrl` 由 `OAuth2ClientProperties.getDefaultRegistrationId()` 动态计算，前端不硬编码授权端点。
...
| 密码过期（登录前） | ✅ | 同初始密码改密流程 |
| 密码过期（会话中） | ✅ | `SessionStateCheckFilter` + SAS 重验，GAP-2 已修复 |
| 登录失败与锁定 | ✅ | 阶梯锁定，审计日志完整 |
...
└─ Order 2: DefaultSecurityConfig（form-login + /api/oauth2/user/** 保护）
...
### Token 配置（`oauth2_registered_client.token_settings`）
...
| 场景 | Status | 响应体 | 处理器 |
|------|--------|--------|--------|
| 登录成功 | **200** | `{"code":null,"data":{"redirectUrl":"<URL>"}}` | `JsonAuthenticationSuccessHandler` |
| 失败（密码错误 / 锁定 / 禁用） | **401** | `{"code":"2003","msg":"<Spring i18n 消息>"}` | `JsonAuthenticationFailureHandler` |

> `redirectUrl` 来自 `HttpSessionRequestCache`（登录前被拦截的 OAuth2 授权请求 URL），前端跳转后完成完整授权码流程。
...
| 场景 | Status | 响应 | 处理器 |
|------|--------|------|--------|
| 授权成功 | **302** | 重定向至 `defaultSuccessUrl` | `SimpleUrlAuthenticationSuccessHandler` |
...
| AT 过期，SAS 刷新时检测到密码 / 账号异常 | **401** | `{"code":"2003","data":{"loginUrl":"..."}}` | `SessionStateCheckFilter` |
...
> 前端全局拦截器（`requestErrorConfig.ts`）收到 401 后显示"登录状态已过期，请重新登录"提示，1.5 秒后跳转 `loginUrl`。**前端不硬编码授权端点**，地址由后端响应动态提供。
...
| 场景 | Status | 响应体 | 处理器 |
|------|--------|--------|--------|
| 正常登出 | **200** | `{"redirectUrl": "/oauth2/authorization/snack-upms-server"}` | `JsonLogoutSuccessHandler` |
...
| UserDetailsService | `snack-oauth-biz/.../service/impl/UserDetailsServiceImpl.java` | `loadUserByUsername()`, `getAuthorities()` |
...
| Profile 控制器 | `snack-oauth-biz/.../controller/OAuthUserProfileController.java` | `updateProfile()` |
| **OAuth2 Client / BFF** | | |
...
| 401 处理器 | `snack-oauth-biz/.../security/handler/BizAuthenticationEntryPoint.java` | `commence()` |
...
| `OAuth2UserServiceImpl.create()` | `setExpired(Status.ENABLED=1)` | `setExpired(YesNoStatus.NO=0)` |
| `OAuth2UserServiceImpl.update()` | `setExpired(Status.ENABLED=1)` | `setExpired(YesNoStatus.NO=0)` |
...
3. ✅ `OAuth2SecurityPolicy` 收紧：`/api/oauth2/user/**` 改为 `hasAuthority("SCOPE_upms")`

**防线**：UPMS RBAC（第一道）→ `SCOPE_upms`（第二道）→ 网络隔离（第三道）

---

### ~~GAP-2~~：密码在活跃会话中过期时前端无法感知 ✅ 已修复

1. ✅ `OAuth2TokenCustomizerImpl`（SAS）：`refresh_token` grant 时重新从 DB 加载用户状态，状态异常则拒绝续发
...
### ~~GAP-3~~：UPMS 后台与 oauth2_user 同步缺失 / 无强制下线接口 ✅ 已修复
...
4. ✅ oauth-server 增加 `DELETE /api/oauth2/user/{username}/tokens`，按 `principal_name` 删除 `oauth2_authorization` 记录（RT 立即失效）
...
`PUT /api/oauth2/user/profile` 接受任意字符串。建议在 `SecurityProperties` 增加复杂度配置（最小长度、大小写/特殊字符要求），在 `OAuth2UserServiceImpl.update()` 中校验。

---

### ~~GAP-6~~：账号自动到期机制缺失 ✅ 已修复

1. ✅ `oauth2_user` 增加 `expire_date date` 字段（null=永不到期）
...
3. ✅ `OAuth2UserDTO` / `SysUserDTO` 增加 `expireDate LocalDate`，管理员通过 `PUT /api/upms/users/{id}` 设置并透传至 oauth2_user

`expired` 字段保留，供管理员手动强制过期；`expire_date` 用于日期自动判定，两者互补。
...
| 字段 | 所在表 | 清除含义 |
|------|--------|---------|
| `expire_date` | oauth2_user | 合同延期，取消到期限制 |
| `mobile` | oauth2_user / sys_user | 解绑手机号（按业务决定是否支持） |
| `email` | oauth2_user / sys_user | 解绑邮箱（按业务决定是否支持） |

当前 `SysUserServiceImpl.update()` 对上述字段均以 `!= null` 为条件同步，一旦设置便无法通过同一接口清除。

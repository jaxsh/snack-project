...
⑤ 用户提交新密码 → PUT /api/oauth/user/profile
...
4. ✅ oauth-server 增加 `DELETE /api/oauth/user/{username}/tokens`，按 `principal_name` 删除 `oauth2_authorization` 记录（RT 立即失效）
...
`PUT /api/oauth/user/profile` 接受任意字符串。建议在 `SecurityProperties` 增加复杂度配置（最小长度、大小写/特殊字符要求），在 `OAuth2UserServiceImpl.update()` 中校验。
